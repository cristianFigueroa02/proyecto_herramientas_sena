...
WHERE 
    prestamos.estado_prestamo = 'prestado'
ORDER BY 
    ...
    $pdf->Cell($anchoPagina, 10, 'Reporte de Prestamos', 0, 1, 'C'); // 0 para el borde, 1 para salto de línea, 'C' para centrado
    ...
    $pdf->Cell($anchoPagina * 0.1, 10, 'Estado Prestamo', 1);
    $pdf->Cell($anchoPagina * 0.1, 10, 'Herramienta', 1);
    ...
    foreach ($datos as $row) {
        ...
        $pdf->Cell($anchoPagina * 0.1, 10, $row["estado_prestamo"], 1);
        $pdf->Cell($anchoPagina * 0.1, 10, $row["nombre_he"], 1);
